...
export const conventionSchema = z.object({
  ...
  etat_conv: z
    ...
    .refine((val) => ['active', 'inactive', 'en_cours', 'terminee', 'suspendue'].includes(val), {
      message: 'État invalide. Valeurs acceptées: active, inactive, en_cours, terminee, suspendue'
    }),
...
export const conventionFormSchema = conventionSchema
  .omit({ id_convention: true })
  .refine((data) => {
    // Validation: le code ne doit pas être générique
    const genericCodes = ['TEST', 'TEMP', 'CONV', 'XXX', 'ABC', '123'];
    return !genericCodes.includes(data.code_convention);
  }, {
    message: "Le code ne peut pas être un code générique (TEST, TEMP, CONV, etc.)",
    path: ["code_convention"]
  })
  .refine((data) => {
    // Validation: cohérence entre montant et intitulé
    const montant = data.montant_conv;
    const intitule = data.intutile_conv.toLowerCase();
    
    // Si le montant est élevé, l'intitulé devrait contenir des mots-clés appropriés
    if (montant > 1000000) {
      const highValueKeywords = ['projet', 'programme', 'développement', 'infrastructure', 'investissement'];
      return highValueKeywords.some(keyword => intitule.includes(keyword));
    }
    return true;
  }, {
    message: "Pour un montant élevé (>1M), l'intitulé devrait mentionner 'projet', 'programme', 'développement', etc.",
    path: ["intutile_conv"]
  })
  .refine((data) => {
    // Validation: format de référence selon l'état
    if (data.etat_conv === 'active') {
      // Les conventions actives doivent avoir une référence avec année
      const currentYear = new Date().getFullYear();
      const yearPattern = new RegExp(`${currentYear}|${currentYear-1}|${currentYear-2}`);
      return yearPattern.test(data.reference_conv);
    }
    return true;
  }, {
    message: "Les conventions actives doivent avoir une référence contenant l'année courante ou récente",
    path: ["reference_conv"]
  });
...
export const CONVENTION_STATES = [
  { value: 'active', label: 'Active', description: 'Convention en vigueur et opérationnelle' },
  { value: 'inactive', label: 'Inactive', description: 'Convention temporairement suspendue' },
  { value: 'en_cours', label: 'En cours', description: 'Convention en cours de négociation' },
  { value: 'terminee', label: 'Terminée', description: 'Convention arrivée à échéance' },
  { value: 'suspendue', label: 'Suspendue', description: 'Convention suspendue par décision' },
] as const;
...
export const validateDateSignature = (date: string): boolean => {
  ...
  const today = new Date();
  const minDate = new Date('1900-01-01');
  return !isNaN(parsedDate.getTime()) && parsedDate >= minDate && parsedDate <= today;
};
