...
interface IndicateursActivitePtbaFormProps {
...
const indicateurSchema = z.object({
  ...
  activite_ptba: z.string().min(1, "L'activité PTBA est requise"),
  code_indicateur_performance: z.string().optional().nullable(),
  abrege_unite: z.number().min(1, "L'unité est requise"),
});

type IndicateurFormData = z.infer<typeof indicateurSchema>;

export default function IndicateursActivitePtbaForm({
  indicateur,
  onClose,
  onSuccess,
}: IndicateursActivitePtbaFormProps) {
    ...
    defaultValues: {
      ...
      abrege_unite:
        typeof indicateur?.abrege_unite === "number"
          ...
          : indicateur?.abrege_unite?.id_unite || undefined,
    },
  });
  ...
  const updateMutation = useMutation({
    mutationFn: (data: IndicateurFormData) =>
      indicateurActivitePtbaService.update(
        indicateur!.id_indicateur_activite!,
        data
      ),
    onSuccess: () => {
  ...
  // Options pour les activités PTBA
  const activiteOptions = activites.map((activite) => ({
    value: activite.code_activite_ptba,
    label: `${activite.code_activite_ptba} - ${activite.intitule_activite_ptba}`,
  }));
        ...
        <Controller
          ...
          render={({ field }) => (
            <SelectInput
              ...
              value={
                field.value
                  ? activiteOptions.find((opt) => opt.value === field.value)
                  : undefined
              }
              onChange={(option) =>
                option && !Array.isArray(option) && field.onChange(option.value)
              }
              error={errors.activite_ptba}
            />
          )}
        />
