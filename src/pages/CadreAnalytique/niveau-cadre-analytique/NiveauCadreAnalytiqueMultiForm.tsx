...
const niveauSchema = z.object({
  nombre_nca: z.number().min(1, "Le nombre est obligatoire"),
  libelle_nca: z.string().min(1, "Le libellé est obligatoire"),
  code_number_nca: z.number().min(1, "Le code numérique est obligatoire"),
...
interface NiveauCadreAnalytiqueMultiFormProps {
  onBack: () => void;
  onSuccess: () => void;
  editingNiveau?: NiveauCadreAnalytique;
}

export default function NiveauCadreAnalytiqueMultiForm({
  onBack,
  onSuccess,
  editingNiveau,
}: NiveauCadreAnalytiqueMultiFormProps) {
    ...
    defaultValues: {
      niveaux:
        isEdit && editingNiveau
          ? [
              {
                nombre_nca: editingNiveau.nombre_nca,
                libelle_nca: editingNiveau.libelle_nca,
                code_number_nca: editingNiveau.code_number_nca,
                type_niveau:
  ...
  const addNiveau = () => {
    append({
      nombre_nca: fields.length + 1,
      libelle_nca: "",
      code_number_nca: fields.length + 1,
      type_niveau: 1,
    });
  };
  ...
  const onSubmit = async (data: MultiNiveauFormData) => {
    ...
    try {
      if (isEdit && editingNiveau) {
        ...
        const niveauData = {
          nombre_nca: data.niveaux[0].nombre_nca,
          libelle_nca: data.niveaux[0].libelle_nca,
          code_number_nca: data.niveaux[0].code_number_nca,
          type_niveau: data.niveaux[0].type_niveau,
        };

        await niveauCadreAnalytiqueService.update(
          editingNiveau.id_nca,
          niveauData
        );
        toast.success("Niveau analytique modifié avec succès");
      } else {
        // Mode création - plusieurs niveaux possibles
        const niveauxToCreate = data.niveaux.map((niveau) => ({
          nombre_nca: niveau.nombre_nca,
        ...
        await Promise.all(
          niveauxToCreate.map((niveau) =>
            niveauCadreAnalytiqueService.create(niveau)
          )
        );

        toast.success(
          `${niveauxToCreate.length} niveau(x) analytique(s) créé(s) avec succès`
        );
      }
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-gray-600">
            {isEdit
              ? "Modifiez les informations du niveau analytique"
              : "Ajoutez un ou plusieurs niveaux analytiques"}
          </p>
        </div>
      </div>
          ...
          {fields.map((field, index) => (
            ...
            >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <Controller
                  ...
                  render={({ field }) => (
                    <Input
                      ...
                      error={errors.niveaux?.[index]?.nombre_nca}
                      onChange={(e) =>
                ...
                {/* Code numérique */}
                <Controller
                  name={`niveaux.${index}.code_number_nca`}
                  control={control}
                  render={({ field }) => (
                    <Input
                      {...field}
                      ...
                      error={errors.niveaux?.[index]?.code_number_nca}
                      onChange={(e) =>
                        field.onChange(parseInt(e.target.value) || 0)
                      }
                      required
                ...
                {/* Libellé */}
                <Controller
                  name={`niveaux.${index}.libelle_nca`}
                  control={control}
                  render={({ field }) => (
                    <Input
                      {...field}
                      ...
                      error={errors.niveaux?.[index]?.libelle_nca}
                      required
                    />
                  )}
                />
